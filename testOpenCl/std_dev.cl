...
__kernel void stdDev(__read_only image2d_t input, __read_only image2d_t means, __write_only image2d_t output) {
	const int2 coord = (int2)(get_global_id(0), get_global_id(1));
	const float mean = read_imagef(means, sampler, coord).x;
	float sum = 0.f;
